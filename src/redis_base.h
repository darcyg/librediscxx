 ...
 *
 * for Integer reply:
 *   'int64_t * _return' is the integer value
...
class RedisBase2
{
  public:
    ...
    int expireat(const std::string& key, int64_t abs_time);
    ...
    virtual bool setex(const std::string& key, int64_t seconds,
        const std::string& value) = 0;
    //_return 1, the key was set
    //_return 0, the key was not set
    ...
    //NOTICE: if return true, and fields.size() == values->size()
